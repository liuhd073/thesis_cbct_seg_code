...
class BottleneckBlock(nn.Module):
    ...
    def forward(self, x1, x2):
        in_shape1 = list(x1.size())
        in_shape2 = list(x2.size())

        # x = torch.cat((x1.view(in_shape1[0], -1), x2.view(in_shape2[0], -1)), dim=1)
        x = x1.view(in_shape1[0], -1)
        y = self.fc1(x)
